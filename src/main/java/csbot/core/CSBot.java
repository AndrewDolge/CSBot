...
/**
 * Main class of CSBot maintains the core functionality of the bot.
 * 
 */
public class CSBot extends ListenerAdapter {
        ...
        for (File jFile : getPluginDirectory().listFiles(jarFilter)) {
            ...
            try (CommandClassLoader commandLoader = new CommandClassLoader(jFile.toURI().toURL());
                ...
                File dataDir = new File(CSBot.getPluginDataDirectory().getCanonicalPath() + File.separatorChar
                        + jFile.toPath().getFileName().toString().substring(0,
                                jFile.toPath().getFileName().toString().lastIndexOf(".")));
                ...
                logger.debug("Loading Plugin: " + propertyLoader.getName() + " Version: " + propertyLoader.getVersion()
                        + "  by " + propertyLoader.getAuthor());
                ...
                while (classes.hasMoreElements()) {
                    ...
                    if (current.getName().endsWith(".class")) {
                        logger.debug("attempting to load: "
                                + current.getName().substring(0, current.getName().length() - 6).replace("/", "."));

                        commandLoader.loadClass(
                                current.getName().substring(0, current.getName().length() - 6).replace("/", "."));
                    }
                }
    ...
    /**
     * Starts up the bot. Initializes the JDA object with the token, and waits for
     * it to connect to discord.
     * 
    ...
    /**
     * Adds the command to the bot. Each command must have a unique trigger. If a
     * command has the same trigger as a command that is already added, it will not
     * be added.
     * 
    ...
    public boolean addCommand(Command toAdd, String dataDir) {
        // add the command
        boolean added = this.commandManager.addCommand(toAdd, dataDir,
    ...
    /**
     * called whenever a discord messsage appears in chat. checks the event for a
     * command, and calls the Message Processor to handle the message.
     * 
    ...
    /**
     * Sets up a default directory heirarchy for the bot.
     * 
     * In the directory of the bot, this method will attempt to create the
     * following: A directory named plugins, to store jar files of plugins A
     * directory named data, to store data associated with each plugin A file named
     * bot.properties, which will hold the token for Discord and other bot
     * properties.
     * 
     */
    public static void setupDirectory() {

        // attempt to create the plugin directory
        if (!CSBot.getPluginDirectory().exists()) {

            if (CSBot.getPluginDirectory().mkdir()) {
                logger.debug("Created Plugin Directory at: " + CSBot.getPluginDirectory().getPath());
            } else {
                logger.debug("Failed to create Plugin Directory at: " + CSBot.getPluginDirectory().getPath());
            }

        } // if

        // attempt to create the data directory
        if (!CSBot.getPluginDataDirectory().exists()) {

            if (CSBot.getPluginDataDirectory().mkdir()) {
                logger.debug("Created data Directory at: " + CSBot.getPluginDataDirectory().getPath());
            } else {
                logger.debug("Failed to create data Directory at: " + CSBot.getPluginDataDirectory().getPath());
            }
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(propertyFile));) {
           ...
           if(propertyFile.exists()){
               writer.write("token=YourTokenHere");
           }
            ...
            logger.error("csbot.Main: failed to create default bot.properties file.");
        }

    }// setup directory

    /**
     * returns a file representation of the directory where the CSBot.jar is
     * executing
     * 
     */
    ...
    /**
     * returns a file representation of the directory where the plugins jars are
     * stored.
     * 
     */
    ...
    /**
     * returns a file representation of the directory where the data subdirectories
     * are stored.
     */
    public static File getPluginDataDirectory() {
        ...
        try {
            ...
            result = new File(
                    CSBot.getApplicationDirectory().getPath() + File.separatorChar + "data" + File.separatorChar);
